...
def searchProducts(urls):
    client = Groq(
        api_key="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",
    )
    products=[]
    for url in urls:
        # print(url)
        ...
        chat_completion = client.chat.completions.create(
            messages=[
                {
                    "role": "user",
                    "content": f'Read the next text and scrapp the name of the products and their prices, discount (id exists) return all as a list of product price and only food: {data} '
        ...
        chat_completion=client.chat.completions.create(
            messages=[
                {
                    "role": "system",
                    "content":"Yor are a text to JSON parser the result is a JSON {products:[ {name: <description>, price: <name>, category: <category>, discount:<discoun>}}]}, you only use the data in the input to create the list, you only respond with the JSON, remove products that are not food"                        
                },
                {
                    "role": "user",
                    "content":text       
